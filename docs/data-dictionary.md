...
| has_been_liquidated | 1 | profile:has_been_liquidated (deprecated) | Deprecated boolean; fallback insolvency indicator only. | Inconsistently present in live responses; absence is None (unknown), never False - do not coerce. |
| has_super_secure_pscs | 1 | profile:has_super_secure_pscs | Company has one or more legally protected ('super secure') PSCs whose details are withheld from the public register. | A legitimate reason for no visible PSC record; read alongside psc_information_state so protected ownership is not misread as none_reported. |
...
| n_officers_id_verified | 2 | officers:items[].identity_verification_details (present) | Officer appointments carrying an ECCTA identity-verification block. | Rollout in progress (2025-2026); absence means unverified-or-not-yet-due, not non-compliance. Counts the block's presence, not a completed identity_verified_on. |
| n_officers_id_verification_due | 2 | derived from officers:items[].identity_verification_details | Appointments with a verification statement due (appointment_verification_statement_due_on) and no completed identity_verified_on. | Rollout in progress; absence of the block means unverified-or-not-yet-due, not non-compliance. Key names as observed live 2026-08 (see AUDIT). |
| n_officers_id_statement_filed | 2 | officers:items[].identity_verification_details.appointment_verification_statement_date | Appointments with a filed appointment-verification statement. | A SEPARATE signal from identity verification: a filed statement does not set identity_verified_on and does not clear n_officers_id_verification_due. Rollout in progress. |
| psc_fetch_status | 1 | pipeline state (PSC list endpoint fetch outcome) | ok / not_found / not_fetched for the persons-with-significant-control list resource. | PIPELINE STATE, never a company signal: it records whether we fetched the resource, not anything about the company. Never appears as flag evidence. The company's PSC posture is psc_information_state + active_psc_statement_codes. In the 2026-08 live batch the psc-statements endpoint returned 404 for every company with no statement filed, consistent with 404 = none filed. |
...
| psc_natures_of_control | 2 | psc:items[].natures_of_control (active records) | Sorted, distinct, comma-joined natures-of-control across ACTIVE PSC records (e.g. ownership-of-shares-25-to-50-percent). | Verbatim enum values, never normalised. Ceased PSC records excluded. |
| n_psc_id_verified | 2 | psc:items[].identity_verification_details (present) | PSC records carrying an ECCTA identity-verification block. | Rollout in progress (2025-2026); absence means unverified-or-not-yet-due, not non-compliance. Counts the block's presence, not a completed identity_verified_on. |
| n_psc_id_verification_due | 2 | derived from psc:items[].identity_verification_details | PSC records with a verification statement due (appointment_verification_statement_due_on) and no completed identity_verified_on. | Rollout in progress; absence of the block means unverified-or-not-yet-due, not non-compliance. Key names as observed live 2026-08 (see AUDIT). |
| n_psc_id_statement_filed | 2 | psc:items[].identity_verification_details.appointment_verification_statement_date | PSC records with a filed appointment-verification statement. | A SEPARATE signal from identity verification: a filed statement does not set identity_verified_on and does not clear n_psc_id_verification_due. Observed live on 07083592 (statement filed, identity_verified_on absent). Rollout in progress. |
